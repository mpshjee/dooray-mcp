/**
 * Create Project Template Tool
 * Create a new task template in a project
 */
...
export const createProjectTemplateSchema = z.object({
  projectId: z.string().describe('Project ID where the template will be created'),
...
export async function createProjectTemplateHandler(args: CreateProjectTemplateInput) {
  try {
    const result = await projectsApi.createProjectTemplate({
...
export const createProjectTemplateTool = {
  name: 'create-project-template',
  description: `Create a new task template in a Dooray project.

Templates are pre-created task structures that help users quickly create tasks with predefined content.
...
**Due Date Flags**:
- dueDateFlag: false → No schedule
- dueDateFlag: true, dueDate: null → Schedule exists but date is TBD
- dueDateFlag: true, dueDate: "2024-01-15T12:00:00+09:00" → Schedule with specific date

**Examples**:
- Simple template: {"projectId": "123", "templateName": "Bug Report"}
- With body: {"projectId": "123", "templateName": "Feature Request", "body": {"mimeType": "text/x-markdown", "content": "## Description\\n\\n## Expected Behavior\\n\\n## Steps to Reproduce"}}
- With defaults: {"projectId": "123", "templateName": "Sprint Task", "priority": "normal", "tagIds": ["tag1"], "isDefault": true}
...
Returns the created template ID.`,
  inputSchema: {
    ...
    properties: {
      projectId: {
        type: 'string',
        description: 'Project ID where the template will be created',
      },
      templateName: {
        type: 'string',
